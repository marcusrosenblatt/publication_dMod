...
Eq.~\eqref{eq:Sv} also holds for general ODEs that do not follow from mass action kinetics. Moreover the system may be explicitly time-dependent and may contain forcings $u(t)$ which e.g.~describe the external stimulation of the system. Last but not least, the solution of eq.~\eqref{eq:Sv} depends on initial values $x_0 = x(t = 0)$. 


%$\dot x = S \cdot v(x,k,u,t)$.
...
% Beobachtungsfunktion einführen
