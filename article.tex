...
\begin{figure}[ht]
	...
	\caption{Output of the prediction function. \textbf{(A)} Prediction of the TCA states. \textbf{(B)} Sensitivities of the three TCA states with respect to the rate parameters.}
...
\begin{figure}[ht]
	...
	\caption{Result of multi-start fitting procedure. \textbf{(A)} Fits have been sorted by increasing least squares value. Four optima were found with almost identical objective value. \textbf{(B)} The parameter values for different optima are shown in different colors }
	\label{fig:mstrust}
\end{figure}
...
\begin{figure}[ht]
	...
	\caption{Result of multi-start fitting procedure with two experimental conditions. \textbf{(A)} Model prediction of the best fit and the simulated data are shown in different colors. \textbf{(B)} Fits have been sorted by increasing least squares value. The lowest value clearly separates from the second plateau. \textbf{(C)} Plotting the parameter values for each of the fits reveals that the second plateau consists of two optima. The lowest plateau however corresponds to a unique optimum.}
...
\begin{figure}[ht]
	...
	\caption{Profile likelihood. \textbf{(A)} Profiles of all parameters. Data- and prior contribution to the total objective value are distinguished by line-type. \textbf{(B)} Parameter paths for the scaling parameter \code{s}.}
	\label{fig:pl}
\end{figure}
