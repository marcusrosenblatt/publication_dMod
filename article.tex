...
Single mechanisms or single processes of a complex system are represented by single terms occurring in the equations of a dynamic model. Parameter estimation can then identify those processes which are crucial to explain the observation.
...
Dynamic models describe systems with states $x$, usually quantifying the involved species, their interaction and evolution over timer. %  as e.g. given by chemical reaction networks.
...
The easiest and one of the most frequent situations is when $\phi$ follows a normal distribution, $\phi\sim\mathcal N(\mu(\theta), \Sigma)$. When measurements are statistically independent, the variance-covariance matrix $\Sigma = {\rm diag}(\sigma_1^2, \dots, \sigma_N^2)$ is diagonal and $\phi$ factorizes. The Likelihood function becomes
\begin{align}
	L(\theta) &= \prod_{i = 1}^N \frac{1}{\sqrt{2\pi\sigma_i^2}} e^{-\frac{\mu_i(\theta)^2}{2\sigma_i^2}} .
	\label{eq:likelihood}
\end{align}
...
A system of ordinary differential equation $\dot x = f(x, p, u, t)$ is represented by a named \code{character} vector of symbolic expressions, the right-hand side of the ODE, involving symbols for the states $x$, the parameters $p$, the forcings $u$ and the keyword \code{time} for $t$. The names of the vector are the state names. The class provided by \pkg{dMod} for such objects is the \code{eqnvec} class which checks whether the character can be parsed and thus, can be interpreted as an equation.
...
\pkg{dMod} makes use of derivatives wherever possible. It is one of the core functionalities of the \pkg{cOde} package upon which \pkg{dMod} is based to augment a system of ODEs by its sensitivity equations. If many parameters are involved and the system consists of many states, the number of equations quickly grows. Solving the equations can be considerably accelerated by utilizing compiled code. \pkg{dMod} provides the \code{odemodel} class. An object of class \code{odemodel} is generated from an \code{eqnlist} or \code{eqnvec}. In the background, \proglang{C} code for the ODE and the combined system of ODE and sensitivity equations is generated, written to the working directory and compiled. The \code{odemodel} object keeps track of the shared objects. It is the basis of a prediction function $x(t, p)$ generated by the \code{Xs()} command.

\subsection{Prediction functions}

\pkg{dMod} seeks to stay close to the mathematical formulation, i.e.~\code{x <- Xs(myodemodel)} will indeed return an \proglang{R} function, an object of class \code{prdfn}, which expects arguments \code{times} and \code{pars} and turns them into a model prediction. However, as indicated by the \code{"s"} in \code{"Xs"}, not only the solution $x(t)$ is returned, the sensitivities $\frac{\partial x}{\partial p}(t)$ are returned, too.
...
The experiment start with all TCA concentrations in steady state, such as shown in Figure~\ref{fig:prediction} after $t = 50$. To initiate the efflux, the buffer is replaced by TCA-free buffer, i.e.~\code{TCA_buffer = 0}. This translates into the following initial parameter values:
...
\end{CodeInput}
\end{CodeChunk}
Since \code{g} and \code{x} have been generated as generic functions, i.e.~\code{condition = NULL}, we can assign the output to a condition of our choice, in thas case "standard".
...
does not conform to our model with 95\% percent certainty.
...
In Section \ref{chap_uncertainty}, profile likelihood computation showed the existence of both practically and structurally non-identifiable parameters. While practical non-identifiabilities might only be removed by additional experiments, structural non-identifiabilties are connected to Lie-group symmetries, i.e. parameter transformations \[\Psi: (x,\theta) \Longrightarrow (x^*, \theta^*)\] that preserve model predictions of the observables: \[g(x^*, \theta^*) = g(x,\theta)\,.\] Based on \cite{merkt2015higher}, the \code{symmetryDetection()} command outputs a list of available symmetry transformations separated into scaling and translational invariances. For example, the code
\begin{CodeChunk}
\begin{CodeInput}
reactions <- NULL
reactions <- addReaction(reactions, "TCA_buffer", "TCA_cell",
                         rate = "import_buffer*TCA_buffer",
                         description = "Uptake")
reactions <- addReaction(reactions, "TCA_cell", "TCA_buffer",
                         rate = "exp_sinus*TCA_cell",
                         description = "Sinusoidal export")
reactions <- addReaction(reactions, "TCA_cell", "TCA_cana",
                         rate = "exp_cana*TCA_cell",
                         description = "Canalicular export")
reactions <- addReaction(reactions, "TCA_cana", "TCA_buffer",
                         rate = "reflux*TCA_cana",
                         description = "Reflux into the buffer")
...
\end{CodeInput}
\end{CodeChunk}
outputs one scaling invariance for the parameters \code{s}, \code{TCA_buffer}, \code{TCA_cana} and \code{TCA_cell} which is in agreement with the identifiability analysis by the method of profile likelihood. Here, the scaling invariance could be removed by fixing one of the parameters to a certain value unequal to zero\footnote{Fixing $\mbox{TCA\_buffer}=0$ does not remove the invariance, since the corresponding symmetry transformation is not valid (\cite{merkt2015higher}).}. For the case of a translational invariance all values were allowed.

\subsection{Analytical steady-state constraints}
While for the the present model, the explicit steady-state implementation could be easily calculated by hand, this might be much more challenging for model classes with high amounts of states and parameters. For many of these cases, the \code{steadyStates()} command outputs an analytical steady-state solution that can be incorporated in the model as an additional parameter transformation. Based on \cite{rosenblatt2016customized}, the steady-state constraint is solved for a combination of state variables and kinetic parameters while positivity of the solution is ensured. As the paper states, the approach outperforms common methods of steady-state implementation with respect to reliability and performance of the optimization process. The corresponding code reads:
\begin{CodeChunk}
\begin{CodeInput}
SS_equations <- steadyStates(reactions)
pSS <- pSS + P(SS_equations)
\end{CodeInput}
\end{CodeChunk}
In addition, \code{steadyStates()} prints conserved quantities that have been found during the calculation. In order to keep the steady-state solution as simple as possible, one of the state variables that contribute to the conserved quantity are left unconstrained.
