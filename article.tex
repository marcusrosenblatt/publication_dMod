...
In this paper, we present the \proglang{R} package \pkg{dMod} building upon \pkg{cOde} to provide a  framework dealing with the inverse problem in dynamic systems. The particularity of the \pkg{dMod/cOde} approach is to provide and propagate accurate derivatives computed from symbolic expressions wherever possible. This derivative information highly supports the convergence of optimization routines and provides the numerical stability required by sofisticated uncertainty analysis methods. Computational efficiency is guaranteed by automatic generation and integration of \proglang{C} code. The framework is object oriented (S3) and provides a variety of functions to set up dynamic models, observation functions and parameter transformations for multi-conditional parameter estimation. 
...
\begin{enumerate}[(i)]
	...
	\item identifiabiliy and uncertainty analysis based on the profile-likelihood method to determine confidence intervals for parameters and predictions.
\end{enumerate}
...
Object of the dynamic modeling are systems with states $x$, usually quantifying the involved species, that evolve over time as e.g. given by chemical reaction networks. The time evolution of the states is expressed via a set of ODEs, $\dot x = f(x)$. 
Typical examples follow the law of mass action:
\begin{itemize}
	\item $\dot x_A = k_p$ constant production of $x_A$ ($\emptyset \rightarrow A$)
	\item $\dot x_C = k_c \cdot x_A x_B = - \dot x_A = - \dot x_B$ complex formation ($A+B \rightarrow C$)
	\item $\dot x_C = - k_d x_C$ constant degradation of $x_C$ ($C \rightarrow \emptyset$)
\end{itemize}
generally written as $\dot x_l = \sum_{j = 1\dots m}  k_{j} \prod_{i = 1\dots n} \nu_{i,j} x_i^{\nu_{i,j}}$ for a system of $n$ states and $m$ reactions with rate constants $p = \{ k_j, j = 1\dots m\}$ and stoichiometric constants $\nu_{i,j} \in \mathbb{Z}$. 
An even more general expression of the system is given by $\dot x = S v(x,p)$ with the stoichiometry matrix $S$ containing the stoichiometric constants $\nu_{i,j}$ and the flux vector $v(x,p)$.
This expression also holds for more general systems that do not follow mass action kinetics. Moreover the system may be explicitly time-dependent and it may contain forcings $u(t)$ which e.g. describe the external stimulation of the system with a compound, resulting in the general form: \begin{align*}\dot x = S \cdot v(x,p,u,t).\end{align*} 

The aim of parameter estimation will be the determination of the free parameters of the system. Besides the rate constants, they include parameters of the forcings $u(t)$ and initial values of the states at time point zero. It is generally assumed that the stoichiometry of the system is known. 
...
\begin{figure}[ht]
	...
	\label{fig:mstrust}
\end{figure}
Despite the similar objective value, the optima are not close to each other in parameter space, as being illustrated by Figure~\ref{fig:mstrust}B. From the initial value parameters \code{TCA_cell} and \code{TCA_cana} it seems that they are exchangable. Where \code{TCA_cell} is high, \code{TCA_cana} is low and vice versa. This is not surprising because the observed cellular TCA amount is the sum of both. A new experiment needs to be designed to distinguish one situation from another.

\subsection{Working with several conditions}\label{sec:conditions}

In practice, the canaliculi only form a closed compartment if Ca$^{2+}$/Mg$^{2+}$ ions are present in the buffer. Therefore, if the experiment is repeated with Ca$^{2+}$/Mg$^{2+}$-free efflux buffer, the contents of the canaliculi escapes quickly into the buffer compartment and the observables \code{buffer} and \code{cellular} effectivly reflect \code{TCA_buffer + TCA_cana} and \code{TCA_cell}. Mathematically, two experimental conditions which differ only by the \code{reflux} parameter need to be combined in one objective function.
...
Compared to the situation before we have gained the outer parameter \code{reflux_open}. Both transformations, "standard" and "open", evaluate the same vector of outer parameters meaning that they yield the same values for all but \code{reflux} and \code{reflux_open}. The prediction function \code{g*x} is generic in the sense that \code{condition = NULL} wheras the concatenation \code{g*x*p} has the conditions "standard" and "open", evaluating the identical function \code{g*x} on two parameter vectors.
