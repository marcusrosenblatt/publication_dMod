...
\noindent In this paper, we present the \pkg{dMod} package which provides an \proglang{R} framework to deal with the inverse problem in dynamic systems. The particularity of the \pkg{dMod} approach is to provide and propagate accurate derivatives computed from symbolic expressions. This derivative information highly supports the convergence of optimization routines and provides the numerical stability required by sofisticated uncertainty analysis methods. Computational efficiency is guaranteed by automatic generation and integration of \proglang{C} code. The framework is object oriented (S3) and provides a variety of functions to set up dynamic models, observation functions and parameter transformations for multi-conditional parameter estimation.

\noindent The key elements of the framework and the methodology implemented in \pkg{dMod} are highlighted by an application on a three-compartment transporter model.
...
There are several ways to define the differential equations. An \code{eqnvec} of equations can be explicitly formulated, analogously to a \code{c()} command. Especially for chemical reactions it can be tideous to keep track of all gain and loss terms. Therefore, \pkg{dMod} provides the \code{eqnlist} class which encodes the ODE by a list of (1) state names, (2) rate expressions, (3) compartment volumes and (4) the stoichiometric matrix. Each reaction flux occurs only once and the gain and loss is represented by the coefficients in the stoichiometric matrix [REF]. \pkg{dMod} supports the read-in of a csv file with the stoichiometric matrix and rate expressions and provides a function \code{addReaction()} to construct an \code{eqnlist} object step-by-step and add further reactions to an already existing \code{eqnlist}. The conversion to the (differential) equations is done by \code{as.eqnvec()} which does all the bookkeeping of gain and loss terms and volume ratios due to compartment transitions.
...
\end{equation*}
All symbols from \code{eqnvec1} and \code{eqnvec2} are collected and their union constitutes the symbols of $\theta$. The evaluation \code{p(theta)} returns a list of length two with inner parameters. The \code{"+"} operator can be applied consecutively to add condition by condition.
...
Objective functions can be added by the \code{"+"} operator, meaning the objective values, gradients and Hessians are added. Besides \code{normL2()}, functions like \code{constraintL2()} and \code{datapointL2()} produce objective functions, too. They allow to define parameter priors or treat data points as parameters.
